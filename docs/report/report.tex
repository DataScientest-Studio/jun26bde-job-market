...
During synchronisation, all active jobs are selected from PostgreSQL together with the fields required for full-text search and filtering. Because one job may have several rows in the \path{job_locations} table, their city values are aggregated into a single \path{cities} array. One Elasticsearch document therefore represents one job advertisement even when the relational record has multiple locations.

The existing index is recreated and the selected jobs are inserted using the PostgreSQL \path{reference_number} as the Elasticsearch document identifier. Only attributes required by the search functionality are copied. The resulting mapping is summarised in Table~\ref{tab:elasticsearch-mapping}.
...
The job-search endpoint distinguishes between free-text search and structured filtering. The API parameter \path{keyword} triggers relevance-oriented full-text search, while parameters such as \path{city}, \path{company}, \path{home_office}, and \path{category} restrict the eligible result set. The API parameter \path{keyword} should not be confused with Elasticsearch's \path{keyword} field type.
