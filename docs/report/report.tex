...
%\usepackage[a4paper,margin=4.5cm]{geometry} \pagestyle{empty} % only for proof reading
\usepackage{xcolor}
...
The right side of Figure~\ref{fig:elasticsearch-search} shows this keyword-search path in more detail. Keyword searches use an Elasticsearch \path{multi_match} query across title, occupation, company, category, and description. These fields are weighted according to their expected relevance, with title receiving the highest boost and description the lowest; the corresponding relevance weights are listed in the figure. Matches in more heavily weighted fields therefore contribute more strongly to the ranking.
